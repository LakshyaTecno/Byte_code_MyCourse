...
exports.createCourse = async (req, res) => {
  try {
    ...
    const engineer = engineerarray.sort(
      (a, b) => a.openTicketsAssigned.length - b.openTicketsAssigned.length
    )[0]; // sorting them and selecting one with least OPEN tickets
    ...
    if (ticketCreated) {
      ...
      console.log(
        `#### New ticket '${ticketCreated.title}' created by ${customer.name} ####`
      );
  ...
  try {
    ...
    if (req.user.userType == constants.userType.customer) {
      ...
      if (req.query.status == constants.ticketStatus.open) {
        //if query params is set to open
        queryObj["$or"] = [
          { _id: { $in: req.user.openTicketsCreated } },
          { _id: { $in: req.user.openTicketsAssigned } },
        ];
      } else if (req.query.status == constants.ticketStatus.closed) {
        //if query params is set to closed
        queryObj["$or"] = [
          { _id: { $in: req.user.closedTicketsCreated } },
          { _id: { $in: req.user.closedTicketsAssigned } },
        ];
      } else {
        //if no query params are provided concat both open and close tickets
        req.user.ticketsCreated = req.user.openTicketsCreated.concat(
          req.user.closedTicketsCreated
        );
        req.user.ticketsAssigned = req.user.openTicketsAssigned.concat(
          req.user.closedTicketsAssigned
        );
        queryObj["$or"] = [
          { _id: { $in: req.user.ticketsCreated } },
          { _id: { $in: req.user.ticketsAssigned } },
        ];
      }
    }
  ...
  try {
    ...
    if (req.body.status && ticket.status != req.body.status) {
      ...
      if (currentTicketStatus == constants.ticketStatus.open) {
        ...
        //if ticket is closed and to be reopened
    ...
    if (req.body.assignee && ticket.assignee != req.body.assignee) {
      //if asignee is non-empty and diffrent
    ...
    console.log(
      `Ticker ${updatedTicket._id} updated by ${req.user.userType} ${req.user.name}`
    );

    sendNotificationReq.ticketUpdated(
      updatedTicket,
      ticketReporter,
      ticketAssignee
    );
